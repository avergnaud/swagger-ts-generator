...
export function generateModelTSFiles(swagger, options) {
...
function getIsUniqueImportType(currentTypeName, baseType, typeProperties) {
    ...
    if (currentTypeName === baseTypeName) {
        ...
        return !typeProperties.some((property) => property.importType === currentTypeName);
}
...
function removeFilesOfNonExistingTypes(typeCollection, folder, options, suffix) {
    ...
    each(files, (file) => {
        if (endsWith(file, suffix) && !typeCollection.find((type) => {
